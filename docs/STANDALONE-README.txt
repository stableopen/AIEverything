AIEverything 1.0.3（Windows x64）
==============================
...
可选 Outlook 邮件索引保存在 %LOCALAPPDATA%\AIEverything\mail.db，默认关闭。

可选 Classic Outlook 邮件搜索：
- 在设置中点击“开启并同步”，只读默认收件箱和已发送中最近 100 封邮件。
...
1.0.3 没有 Local Import、剪贴板、Teams/WeChat 或手工目录入口。
