...
![AIEverything 1.0.4 Word 正文搜索](docs/images/aieverything-1.0.4-docx.png)

AIEverything 是独立开源项目，不是 voidtools 官方产品，也不隶属于 voidtools。文件名能力基于随包分发的 Everything 1.4 引擎和 SDK；相关版权与许可证归原作者所有。

## 1.0.4 核心功能

- 支持全盘文件名与路径极速搜索，基于 Everything 索引快速定位本机文件。
- 支持 TXT、Markdown、Word 文档正文搜索，可直接定位所需内容。
- 支持 Outlook 邮件搜索，快速查找历史邮件及相关信息。
- 支持 AI 智能排序，结合本地 MiniLM 与使用习惯优化搜索结果；坚持本地优先，不修改用户源文件。
...
### 4. 自动搜索 Classic Outlook 邮件

1.0.4 默认在启动后静默只读同步 Classic Outlook 默认收件箱和已发送中最近 100 封邮件。主题、人员、正文片段和附件名称保存在本机 `mail.db`；“全部/正文”可搜索邮件，双击可回到 Outlook 原邮件。不发送、删除、移动或修改邮件；不需要邮件搜索时可在设置中关闭并清除本机索引。

## 快速开始

直接下载 [AIEverything-1.0.4-win-x64.zip](https://github.com/stableopen/AIEverything/releases/download/v1.0.4/AIEverything-1.0.4-win-x64.zip)，解压到普通可写目录后双击 `AIEverything.exe`。无需安装 .NET SDK。

开始使用：

1. 下载并解压便携 ZIP。
2. 双击 `AIEverything.exe`，文件名搜索立即可用。
3. Word、TXT、Markdown 正文和 Classic Outlook 最近邮件会在后台自动加入搜索；搜索期间界面仍可使用。

校验文件：[SHA-256](https://github.com/stableopen/AIEverything/releases/download/v1.0.4/AIEverything-1.0.4-win-x64.zip.sha256)。遇到问题请[直接反馈](https://github.com/stableopen/AIEverything/issues/new)。源码构建：
...
构建完成后，解压 `dist\AIEverything-1.0.4-win-x64.zip` 到普通可写目录，双击 `AIEverything.exe`。文件名搜索立即可用，正文和 Classic Outlook 最近邮件默认在后台自动加入搜索；可在设置中暂停或关闭。
...
| 邮件正文 | Classic Outlook 默认收件箱和已发送，最近 100 封；默认启动同步 |
...
- Classic Outlook 最近邮件索引保存在 `%LOCALAPPDATA%\AIEverything\mail.db`，位于本机且未单独加密，可在设置中关闭或清除。
...
- PDF、旧版 DOC、RST、XLSX、PPTX、OCR 和聊天内容不在 1.0.4 正文索引范围；邮件不读取附件正文。
